    ...
    for (i = 0; i < murl->size && murl->num > 0; i++)
    {
        if (conns[i].status != MURLS_NONEXIST)
        {
            ...
            if (should_cleanup == 0)
            {
                ...
                curl_easy_setopt(conns[i].easy, CURLOPT_COOKIE, NULL);
                curl_easy_setopt(conns[i].easy, CURLOPT_HTTPHEADER, NULL);
            }
            else if (should_cleanup == 2)
            {
                // curl_easy_reset(conns[i].easy);
                curl_easy_setopt(conns[i].easy, CURLOPT_COOKIE, NULL);
                curl_easy_setopt(conns[i].easy, CURLOPT_HTTPHEADER, NULL);
            ...
            conns[i].error[0] = '\0';
            free((char *)conns[i].cookie);
            curl_slist_free_all(conns[i].header);
            conns[i].header = NULL;
...
//    memset(buf, 0, size);
    *(uint32_t *)buf = 0;
    ...
    // to avoid dns resolve timeout bug
    curl_easy_setopt(conns[i].easy, CURLOPT_NOSIGNAL, 1);
    if (cookie != NULL)
        curl_easy_setopt(conns[i].easy, CURLOPT_COOKIE, conns[i].cookie);
    if (token != NULL)
    {
        conns[i].header = curl_slist_append(conns[i].header, token);
        curl_easy_setopt(conns[i].easy, CURLOPT_HTTPHEADER, conns[i].header);
    }
...
int murl_remove_url(struct murl_t *murl, const char *url)
{
    ...
    curl_easy_setopt(conn->easy, CURLOPT_COOKIE, NULL);
    free((char *)conn->cookie);
    conn->cookie = NULL;

    curl_slist_free_all(conn->header);
    conn->header = NULL;
        ...
        const char *cookie, const char *token, const char *args)
{
    ...
    if (token != NULL)
    {
        ...
        curl_easy_setopt(conn.easy, CURLOPT_HTTPHEADER, header);
    }
    if (args != NULL) {
        curl_easy_setopt(conn.easy, CURLOPT_POSTFIELDS, args);
    }
...
struct murl_t *murl_malloc_long_connection(int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        murl->conns[i].status = MURLS_NONEXIST;
        murl->conns[i].header = NULL;
        murl->conns[i].easy = curl_easy_init();

        curl_easy_setopt(murl->conns[i].easy, CURLOPT_WRITEFUNCTION, write_cb);
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_WRITEDATA, &(murl->conns[i]));
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_ERRORBUFFER, murl->conns[i].error);
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_PRIVATE, &(murl->conns[i]));
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_CONNECTTIMEOUT_MS, 300);
        // to avoid dns resolve timeout bug
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_NOSIGNAL, 1);
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_TCP_NODELAY, 1);

        curl_easy_setopt(murl->conns[i].easy, CURLOPT_TCP_KEEPALIVE, 1L);
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_TCP_KEEPINTVL, 2L);
        curl_easy_setopt(murl->conns[i].easy, CURLOPT_TCP_KEEPIDLE, 60L);
    }
    murl->size = size;
    murl->num = 0;
...
int murl_remove_url_long_connection(struct murl_t *murl, const char *url)
{
    ...
    curl_easy_setopt(conn->easy, CURLOPT_HTTPHEADER, NULL);
    curl_slist_free_all(conn->header);
    conn->header = NULL;
