...
bool OfoCrm::Run(int i, QueueKafka* queue_kafka, long offset) {
    try {
        ...
        if (i < 40) {
            kafka_consumer_client_ = std::make_shared<kafka_consumer_client>("192.168.30.236:9092", "userevents", "crm_noah_kenan", -1, i);
        } else {
            kafka_consumer_client_ = std::make_shared<kafka_consumer_client>("192.168.30.236:9092", "crm_order_action", "crm_noah_kenan_order", -1, i-40);
        }
